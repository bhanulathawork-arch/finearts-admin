...
//       if (location.pathname === "/trainers/pending") {
//         res = await getPendingTrainers(token);
//       } else if (location.pathname === "/trainers/rejected") {
//         res = await getRejectedTrainers(token);
//       } else {
//         res = await getAdminTrainers(token);
//       }
...
//     try {
//       await deleteAdminTrainer(trainerToDelete, token);
...
//       if (editingTrainer) {
//         await updateAdminTrainer(editingTrainer.id, fd, token);

//         toast.success("Trainer updated successfully");
//       } else {
//         await createAdminTrainer(fd, token);
...
//     await approveTrainer(trainer.id, token);
...
//     await rejectTrainer(trainer.id, "Rejected by Admin", token);
...
//   const getPageTitle = () => {
//     if (location.pathname === "/trainers/pending") return "Pending Trainers";
//     if (location.pathname === "/trainers/rejected") return "Rejected Trainers";
//     return "Admin Trainers";
//   };

//   const getPageSubtitle = () => {
//     if (location.pathname === "/trainers/pending")
//       return "Review and approve trainer applications";
//     if (location.pathname === "/trainers/rejected")
//       return "Manage rejected trainer applications";
//     return "Manage Admin Trainers";
//   };
...
//   // Note: approveTrainer / rejectTrainer were used in your original code
//   // but not imported. If you have them in trainerService, uncomment below.
//   //
//   // const handleApprove = async (trainer) => {
//   //   await approveTrainer(trainer.id, token);
//   //   toast.success("Trainer approved");
...
//   const isPendingPage = location.pathname === "/trainers/pending";
...
//                         {/* Approve / Reject buttons on pending page */}
//                         {isPendingPage && (
//                           <>
//                             <button
//                               onClick={() => {
//                                 // handleApprove(trainer);
//                                 toast("Approve function not connected");
//                               }}
...
//                                 toast("Reject function not connected");
//                               }}
...
//       String(inst.total_reviews).includes(query) ||
//       t.certifications?.toLowerCase().includes(query) ||
...
//   // Updated total column count (original 13 + 3 new = 16)
...
//                     {/* Certifications */}
//                     <td className="p-4 text-gray-400 max-w-[140px] truncate" title={trainer.certifications || ""}>
    ...
    <div className="p-8 text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-white mt-2">{getPageSubtitle()}</p>
        </div>
                ...
                filteredTrainers.map((trainer) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-white whitespace-nowrap">
                      {trainer.email ? (
                        <span className="text-white hover:text-purple-300 transition-colors" title={trainer.email}>
                          {trainer.email.length > 22
                            ? trainer.email.substring(0, 22) + "..."
                            : trainer.email}
                        </span>
                    ...
                    <td className="p-4 font-semibold whitespace-nowrap">
                      {trainer.rating ? Math.round(Number(trainer.rating)) : "-"}
                    </td>
          ...
          <div className="w-full max-w-[650px] max-h-[90vh] bg-[#211c30] rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="overflow-y-auto max-h-[75vh] p-6">
              <form onSubmit={handleSubmit} className="space-y-1">
                ...
                {/* Rating & Reviews — same pattern as Institutes */}
