...
//   /* ─────────────────────────────────────────────
//      LOAD CLASSES (TRAINER SPECIFIC)
//   ───────────────────────────────────────────── */
//   const fetchClasses = async () => {
//     try {
//       const config = await getConfig();
//       const res = await axios.get(`${API_URL}/classes/trainer/my-classes`, config);
...
//   /* ─────────────────────────────────────────────
//      LOAD SESSIONS (TRAINER SPECIFIC)
//   ───────────────────────────────────────────── */
//   const fetchSessions = async () => {
//     try {
//       setLoading(true);
//       const config = await getConfig();
//       const res = await axios.get(`${API_URL}/sessions/trainer/my-sessions`, config);
...
//       data = data.filter((item) => item.title?.toLowerCase().includes(keyword) || item.class_title?.toLowerCase().includes(keyword));
//     }
//     return data;
...
//       const config = await getConfig();
//       await axios.post(`${API_URL}/sessions/trainer/create`, { sessions: payloads }, config);
...
//       setEditing(true);
//       const config = await getConfig();
//       await axios.put(`${API_URL}/sessions/trainer/${editForm.session_id}`, {
...
//       setDeleting(true);
//       const config = await getConfig();
//       await axios.delete(`${API_URL}/sessions/trainer/${selectedSession.id}`, config);
...
//       const config = await getConfig();
//       await axios.post(`${API_URL}/sessions/trainer/${selectedSession.id}/generate-zoom`, {}, config);
...
//             My Sessions
//           </h1>
//           <p className="text-slate-400 mt-2">Manage your live class sessions and Zoom meetings.</p>
//         </div>
...
//   /* CREATE FORM - CHANGED: Removed title, added it to slots. Removed date */
//   const [form, setForm] = useState({ class_id: "", notes: "" });
//   const [slots, setSlots] = useState([
//     { id: Date.now(), title: "", start_time: "", start_ampm: "AM", end_time: "", end_ampm: "AM" },
//   ]);

//   /* EDIT FORM - CHANGED: Removed session_date */
//   const [editForm, setEditForm] = useState({
//     session_id: "", class_id: "", title: "", start_time: "", start_ampm: "AM", end_time: "", end_ampm: "AM", notes: "",
//   });
...
//   ───────────────────────────────────────────── */
//   const addSlot = () => {
//     setSlots((prev) => [...prev, { id: Date.now(), title: "", start_time: "", start_ampm: "AM", end_time: "", end_ampm: "AM" }]);
//   };
...
//   /* ─────────────────────────────────────────────
//      DASHBOARD STATS - CHANGED: Removed cancelled, use live_status
//   ───────────────────────────────────────────── */
//   const stats = useMemo(() => {
//     const list = Array.isArray(sessions) ? sessions : [];
...
//       if (invalidSlot) return toast.error("Please fill name, start time and end time for all templates");
...
//             My Session Templates
//           </h1>
//           <p className="text-slate-400 mt-2">Manage your recurring time slots and Zoom links.</p>
//         </div>
//         <button onClick={() => setShowCreateModal(true)} className="sd-btn flex items-center gap-2 bg-violet-600 hover:bg-violet-700 px-5 py-3 rounded-xl font-semibold">
...
//       {/* DASHBOARD STATS - CHANGED: Removed Cancelled */}
//       <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
//         <div className="sd-card p-5"><p className="text-slate-400 text-sm">Total Templates</p><h2 className="text-4xl font-black mt-2">{stats.total}</h2></div>
//         <div className="sd-card p-5"><p className="text-slate-400 text-sm">Scheduled/Upcoming</p><h2 className="text-4xl font-black text-blue-400 mt-2">{stats.scheduled}</h2></div>
//         <div className="sd-card p-5"><p className="text-slate-400 text-sm">Live Now</p><h2 className="text-4xl font-black text-green-400 mt-2">{stats.live}</h2></div>
//         <div className="sd-card p-5"><p className="text-slate-400 text-sm">Completed Today</p><h2 className="text-4xl font-black text-cyan-400 mt-2">{stats.completed}</h2></div>
//       </div>
...
//           <div className="relative">
//             <Search size={18} className="absolute left-4 top-3.5 text-slate-500" />
//             <input type="text" placeholder="Search templates..." value={search} onChange={(e) => setSearch(e.target.value)} className="sd-input w-full rounded-xl pl-11 pr-4 py-3" />
//           </div>
//           <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="sd-input rounded-xl px-4 py-3">
//             <option value="ALL">All Status</option>
...
//           <div className="py-24 flex flex-col items-center justify-center">
//             <Loader2 className="animate-spin text-violet-500" size={42} />
//             <p className="mt-5 text-slate-400">Loading Templates...</p>
//           </div>
//         ) : paginatedSessions.length === 0 ? (
//           <div className="py-24 text-center">
//             <BookOpen size={64} className="mx-auto text-slate-600" />
//             <h3 className="mt-6 text-2xl font-bold">No Templates Found</h3>
//             <p className="mt-2 text-slate-400">Create your first time slot template to begin.</p>
//             <button onClick={() => setShowCreateModal(true)} className="mt-8 px-6 py-3 rounded-xl bg-violet-600 hover:bg-violet-700">
//               <Plus size={18} className="inline mr-2" /> Create Template
//             </button>
//           </div>
//         ) : (
//           <>
...
//                     <th className="px-6 py-4 text-left text-sm font-semibold">Class</th>
//                     {/* REMOVED DATE COLUMN */}
//                     <th className="px-6 py-4 text-left text-sm font-semibold">Time Window</th>
//                     <th className="px-6 py-4 text-left text-sm font-semibold">Status</th>
//                     <th className="px-6 py-4 text-left text-sm font-semibold">Zoom</th>
//                     <th className="px-6 py-4 text-right text-sm font-semibold">Actions</th>
//                   </tr>
...
//                     const start12 = convertTo12Hour(session.start_time);
//                     const end12 = convertTo12Hour(session.end_time);
//                     // CHANGED: Fallback to handle both status and live_status dynamically
...
//                           }`}>
//                             {currentStatus === "LIVE" && <CheckCircle size={14} />}
//                             {currentStatus}
//                           </span>
//                         </td>
//                         <td className="px-6 py-5">
...
//       {/* ============================================================
//           CREATE SESSION MODAL (TEMPLATES)
//       ============================================================ */}
//       <OverlayModal open={showCreateModal} onClose={() => setShowCreateModal(false)}>
//         <div className="sd-card w-full max-w-3xl p-8 max-h-[90vh] overflow-y-auto">
//           <div className="flex items-center justify-between mb-8">
//             <div>
//               <h2 className="text-3xl font-bold">Create Session Templates</h2>
//               <p className="text-slate-400 mt-2">Define recurring time slots (e.g., Session A, Session B).</p>
//             </div>
//             <button onClick={() => setShowCreateModal(false)} className="text-slate-400 hover:text-white"><X size={22} /></button>
//           </div>
//           <div className="space-y-6">
...
//             <div>
//               <label className="block mb-2 text-sm font-medium">Notes</label>
//               <textarea rows={3} className="sd-input w-full rounded-xl p-3" placeholder="General Notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
//             </div>
//             <div className="border-t border-white/10 pt-6">
//               <h3 className="text-lg font-bold flex items-center gap-2 mb-4"><Clock size={18} className="text-violet-400" /> Time Slots</h3>
//               <div className="space-y-3">
//                 {slots.map((slot) => (
//                   <div key={slot.id} className="sd-card p-4">
//                     <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
//                       {/* CHANGED: Date to Template Name */}
//                       <div>
//                         <label className="block mb-1.5 text-xs text-slate-400">Template Name</label>
//                         <input type="text" className="sd-input w-full rounded-lg p-2.5 text-sm" placeholder="e.g. Session B" value={slot.title} onChange={(e) => updateSlot(slot.id, "title", e.target.value)} />
//                       </div>
//                       <div>
//                         <label className="block mb-1.5 text-xs text-slate-400">Start Time</label>
//                         <div className="flex gap-2">
...
//               <p className="text-slate-400 mt-2">Update time slot information.</p>
//             </div>
//             <button onClick={() => setShowEditModal(false)} className="text-slate-400 hover:text-white"><X size={22} /></button>
//           </div>
//           <div className="space-y-6">
...
//             {/* CHANGED: Removed Date Input, only Time Inputs remain */}
//             <div className="border-t border-white/10 pt-6">
//               <h3 className="text-lg font-bold flex items-center gap-2 mb-4"><Clock size={18} className="text-violet-400" /> Time Window</h3>
//               <div className="sd-card p-4">
...
//       {/* ============================================================
//           DELETE CONFIRMATION MODAL - CHANGED: Removed Date Display
//       ============================================================ */}
//       <OverlayModal open={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
//         <div className="sd-card max-w-lg w-full p-8">
//           <div className="flex items-center gap-4 mb-6">
//             <div className="w-14 h-14 rounded-full bg-red-500/20 flex items-center justify-center"><Trash2 size={28} className="text-red-400" /></div>
//             <div>
//               <h2 className="text-2xl font-bold">Delete Template</h2>
//               <p className="text-slate-400 mt-1">This action cannot be undone.</p>
//             </div>
//           </div>
//           <div className="rounded-xl bg-slate-900 p-5 border border-slate-700">
//             <p className="text-slate-300">Are you sure you want to delete this time slot template?</p>
//             {selectedSession && (
//               <div className="mt-4 space-y-2">
//                 <div><span className="text-slate-500">Title</span><p className="font-semibold">{selectedSession.title || "Untitled Template"}</p></div>
//                 {/* REMOVED DATE DIV */}
//                 <div><span className="text-slate-500">Time Window</span><p>{convertTo12Hour(selectedSession.start_time).time} {convertTo12Hour(selectedSession.start_time).ampm} - {convertTo12Hour(selectedSession.end_time).time} {convertTo12Hour(selectedSession.end_time).ampm}</p></div>
//               </div>
//             )}
//           </div>
//           <div className="flex justify-end gap-4 mt-8">
...
//             <div>
//               <h2 className="text-3xl font-bold flex items-center gap-3"><Video className="text-violet-400" /> Zoom Meeting</h2>
//               <p className="text-slate-400 mt-2">Manage the recurring Zoom link for this template.</p>
//             </div>
//             <button onClick={() => setShowZoomModal(false)} className="text-slate-400 hover:text-white text-2xl"><X size={22} /></button>
//           </div>
//           {selectedSession?.zoom_link ? (
...
//               <Video size={70} className="mx-auto text-slate-600" />
//               <h3 className="text-2xl font-bold mt-6">No Zoom Meeting</h3>
//               <p className="text-slate-400 mt-2">Generate a recurring Zoom link for this time slot.</p>
//             </div>
//           )}
//           <div className="flex justify-end gap-4 mt-10">
//             <button onClick={() => setShowZoomModal(false)} className="px-6 py-3 rounded-xl border border-slate-700 hover:bg-slate-800">Close</button>
...
//                   Define time slots (e.g., Session A, Session B).
//                 </p>
//               </div>
//               <button
